...
function mousePressed() {
  ...
  const startTime = Tone.now();
  const baseTimeStep = 0.5; // Base time between notes
  const quadrantDelay = 1; // Delay between quadrants

  // Define pitch range for the notes
  const pitchMin = 48; // C3
  const pitchMax = 72; // C5

  // Helper function to map tile.type to a MIDI pitch within the range
  const mapPitch = (type) => {
    const scaleLength = scales[0].length; // Assume the first scale for mapping
    const offset = type % scaleLength; // Map type to a scale offset
    return Math.min(
      pitchMax,
      Math.max(pitchMin, instruments.TL.note + offset) // Ensure within pitch range
    );
  };
  ...
  // Group tiles into quadrants dynamically
  const quadrants = {
  ...
  Object.keys(instruments).forEach((key, quadrantIndex) => {
    ...
    tilesInQuadrant.forEach((tile, tileIndex) => {
      const relativeSpeed = 1 - tile.y / height; // Inverse of Y position for rhythm
      if (relativeSpeed === null) return; // Skip rest

      const pitch = mapPitch(tile.type); // Map tile.type to pitch
      ...
      setTimeout(() => {
        ...
        const volume = -12 + (tile.y / height) * 24; // Map Y position to volume
        const pan = map(tile.x, 0, width, -1, 1); // Map X position to pan
        instrument.set({ volume, pan });
  ...
  console.log("Playing dynamically linked rhythm and visuals with controlled pitch.");
}